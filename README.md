...
|------------------------|-----------------------|-------------------------|------------------------------------|
| `assets/agents/*.md`   | `.claude/agents/*.md` | `.opencode/agents/*.md` | `.github/chatmodes/*.chatmode.md`  |
| `assets/skills/<n>/`   | `.claude/skills/<n>/` | `.opencode/skills/<n>/` | — (no SKILL.md mechanism)          |
| `assets/workflows/*.md`| `.claude/commands/*.md` | `.opencode/commands/*.md` | `.github/prompts/*.prompt.md`   |

Notes:
- **Skills** use the shared `SKILL.md` convention, so Claude Code and OpenCode link the
  skill directory directly. Copilot has no equivalent per-skill mechanism today, so skills
  are not linked there; the same intent is available via the chat modes and prompts.
- **Agents** become Claude Code / OpenCode subagents and Copilot **chat modes**.
- **Workflows** become Claude Code / OpenCode slash commands and Copilot **prompt files**.

## Workflow for maintaining the assets

1. Edit or add files under `assets/` here.
...
- **Agent:** add `assets/agents/<name>.md` with `name`, `description`, `mode`, `tools`.
- **Skill:** add `assets/skills/<name>/SKILL.md` with `name` + `description` frontmatter.
- **Workflow:** add `assets/workflows/<name>.md` with `description` + `mode: agent`.
